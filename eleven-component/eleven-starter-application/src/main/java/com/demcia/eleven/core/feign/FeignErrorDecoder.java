...
import com.demcia.eleven.core.exception.ProcessErrorException;
            ...
            for (String type : response.headers().get(HttpHeaders.CONTENT_TYPE)) {
                if (MediaType.parseMediaType(type).isCompatibleWith(MediaType.APPLICATION_JSON)) {
                    try (InputStream is = response.body().asInputStream()) {
                        var failure = objectMapper.readValue(is, RestFailure.class);
                        return ProcessFailureException.of(failure.getError(), failure.getMessage());
                    } catch (IOException e) {
        ...
        if (httpStatus.is5xxServerError()) {
            for (String type : response.headers().get(HttpHeaders.CONTENT_TYPE)) {
                if (MediaType.parseMediaType(type).isCompatibleWith(MediaType.APPLICATION_JSON)) {
                    try (InputStream is = response.body().asInputStream()) {
                        var failure = objectMapper.readValue(is, RestFailure.class);
                        return ProcessErrorException.of(failure.getError(),"内部服务调用失败:"+failure.getMessage());
                    } catch (IOException e) {
                        log.error("feign error decode error", e);
